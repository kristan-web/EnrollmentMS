...
require_once __DIR__ . '/../../Dao/cashier/CashierDAO.php';
...
if ($method === 'POST') {
    ...
    if ($formType === 'login') {
        ...
        // Relative to accounting/index.html, the login page that posts here.
        echo json_encode([
            "success"  => true,
            "message"  => "Signed in. Redirecting to the cashier console…",
            "redirect" => "views/cashier.html"
        ]);
        exit;
    }
    ...
    if ($formType === 'logout') {
        $_SESSION = [];
        session_destroy();
        // Relative to accounting/views/cashier.html, the only page that logs out.
    ...
    if ($action === 'record_payment') {
        ...
        echo json_encode([
            "success"    => true,
            "message"    => $fullyPaid
                ? "Payment recorded. This student is now Fully Paid."
