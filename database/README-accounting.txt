...
Files
-----
  01_accounting_tables.sql   creates acct_fees and acct_payments
  02_fee_schedule.sql        fills acct_fees with the starting fees
...
3. Import the main schema first if you haven't:  database/schema.sql
...
5. Import tab -> choose database/01_accounting_tables.sql -> Go
6. Import tab -> choose database/02_fee_schedule.sql -> Go

Or from the command line:

  cd C:\xampp\mysql\bin
  mysql -u root enrollment_management_system < ...\database\01_accounting_tables.sql
  mysql -u root enrollment_management_system < ...\database\02_fee_schedule.sql
...
that prices an enrollment follows on the next load:
  ...
  acct_fees  ->  Accounting/DAO/AcctFeeDAO.php  ->  FeeSchedule (Accounting/Model/)
                                                |
        +---------------------------------------+----------------------+
        |                        |                                     |
  student SOA + checkout   cashier assessment / balance      registrar payment gate
  (Accounting/View/)       (Accounting/Controller/)          (Registrar/Controller/)
...
built-in list in Accounting/Model/payment_model.php. That keeps a database that never
...
Accounting/DAO/AcctPaymentDAO.php: a 'pending' row when the PayMongo checkout link is
...
---------------------------
public/assets/js/Accounting/accounting-model.js keeps FALLBACK_FEES. It is only rendered
...
Notes
-----
- The tables are prefixed acct_ on purpose. database/schema.sql already has
