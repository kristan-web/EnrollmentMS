// cashier-login.js — Accounting-side (cashier) login; drives accounting/index.html,
// the module's entry point.
// Talks to Controllers/cashier/cashier_controllers.php via CashierModel. The
// credentials are checked against the staff `users` table; on a valid account a
// PHP cashier session is started and the user is sent to views/cashier.html.
...
(function () {
  ...
  M.redirectIfAuthed("views/cashier.html");
  ...
  loginForm.addEventListener("submit", function (e) {
    ...
    M.login(email, password).then(function (data) {
      if (data && data.success) {
        ...
        window.location.href = data.redirect || "views/cashier.html";
        return;
      }
