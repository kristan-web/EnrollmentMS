...
class CashierDAO {
    ...
    // Flowchart: "Fully Paid? -> update status". Once the assessment is fully
    // settled at the cashier, confirm the seat and move the student into the
    // registrar's Enrolled tab — the same end-state the registrar's Finalize
    // produces, so a walk-in/online student who has paid in full doesn't get
    // stranded at 'Approved'. Idempotent: only a 'Pending' enrollment and its
    // 'Approved' applicant are touched, so re-running (or a later Finalize
    // click) is a harmless no-op.
    public function markEnrolledOnFullPayment($enrollmentId, $studentId) {
        // Confirm the enrollment (mirrors RegistrarDAO::finalizeEnrollment).
        $e = $this->conn->prepare(
            "UPDATE enrollments
                SET status = 'Enrolled', date_enrolled = NOW()
              WHERE enrollment_id = :id AND status = 'Pending'"
        );
        $e->bindValue(":id", $enrollmentId);
        $e->execute();

        // Move the linked applicant into the Enrolled tab. The applicant is
        // linked to the student via converted_student_id; only flip it if it
        // was sitting at 'Approved' (the state Finalize also requires).
        $a = $this->conn->prepare(
            "UPDATE applicants
                SET status = 'Enrolled'
              WHERE converted_student_id = :student_id AND status = 'Approved'"
        );
        $a->bindValue(":student_id", $studentId);
        return $a->execute();
    }
