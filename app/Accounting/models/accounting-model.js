...
(function () {
  ...
  /* index.html sits in accounting/ but the other pages sit in accounting/views/,
     so the path to api/ depends on which page loaded this. */
  var inViews = location.pathname.indexOf("/views/") !== -1;
  var API_BASE = (inViews ? "../" : "") + "api/Controllers/payments_controllers.php";
  ...
  /* Display-only backup used when the server is unreachable.
     Keep in sync with FeeSchedule::items() in api/Models/payment_model.php,
