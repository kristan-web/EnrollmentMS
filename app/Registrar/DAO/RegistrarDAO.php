...
require_once "$projectFilePath/app/Registrar/Model/admission_model.php";
...
class RegistrarDAO {
    ...
    // Reference number like "EMS-2026-483920", checked unique before returning.
    // Mirrors the online application flow so walk-ins carry the same style of
    // reference the registrar already recognises in the queue.
    public function generateUniqueReferenceNumber() {
        do {
            $candidate = "EMS-" . date("Y") . "-" . str_pad((string) random_int(0, 999999), 6, "0", STR_PAD_LEFT);
            $stmt = $this->conn->prepare("SELECT 1 FROM applicants WHERE reference_number = :ref LIMIT 1");
            $stmt->bindValue(":ref", $candidate);
            $stmt->execute();
    ...
    // Creates the applicants row that anchors a walk-in in the review queue.
    // A walk-in skips the online application, but the queue, the detail view,
    // and Finalize are all keyed on `applicants` — so without this row the
    // student is saved yet never shows up to be reviewed or finalized. The row
    // is recorded already-'Approved' (the registrar met the student and
    // reserved a seat) and linked to the student we just created.
    // `$d` is the raw walk-in $_POST; `$reviewerId` is the acting registrar.
    // Returns the new applicant_id, or false on failure.
    public function createWalkInApplicant(array $d, $studentId, $schoolYear, $reviewerId) {
        $clean = function ($v) {
        ...
        (
            ...
            status, reviewed_by, reviewed_at, converted_student_id
        )
        VALUES
        (
            :reference_number, :applicant_type, :first_name, :last_name, :middle_name,
            ...
            :guardian_name, :guardian_relationship, :guardian_contact_number,
            :emergency_contact_name, :emergency_contact_relationship, :emergency_contact_number,
            'Approved', :reviewed_by, NOW(), :converted_student_id
        )
        ";
        $stmt = $this->conn->prepare($query);
        $stmt->bindValue(":reference_number", $this->generateUniqueReferenceNumber());
        // The walk-in form doesn't ask New Student vs Transferee; default to the
        // same column default the schema uses.
        $stmt->bindValue(":applicant_type", "New Student");
        $stmt->bindValue(":first_name", trim($d["firstName"]));
        $stmt->bindValue(":last_name", trim($d["lastName"]));
        $stmt->bindValue(":middle_name", $clean($d["middleName"] ?? null));
        $stmt->bindValue(":gender", $d["gender"]);
        $stmt->bindValue(":birthdate", date("Y-m-d", strtotime($d["birthdate"])));
        $stmt->bindValue(":address", trim($d["address"]));
        $stmt->bindValue(":contact_number", $clean($d["contact"] ?? null));
        $stmt->bindValue(":email", trim($d["email"]));
        $stmt->bindValue(":lrn", trim($d["lrn"]));
        $stmt->bindValue(":desired_grade_level", $d["gradeLevel"]);
        $stmt->bindValue(":desired_strand_id", (int) $d["strand_id"]);
        $stmt->bindValue(":school_year", $schoolYear);
        $stmt->bindValue(":father_name", $clean($d["fatherName"] ?? null));
        $stmt->bindValue(":father_contact_number", $clean($d["fatherContact"] ?? null));
        $stmt->bindValue(":mother_name", $clean($d["motherName"] ?? null));
        $stmt->bindValue(":mother_contact_number", $clean($d["motherContact"] ?? null));
        $stmt->bindValue(":guardian_name", $clean($d["guardianName"] ?? null));
        $stmt->bindValue(":guardian_relationship", $clean($d["guardianRelationship"] ?? null));
        $stmt->bindValue(":guardian_contact_number", $clean($d["guardianContact"] ?? null));
        $stmt->bindValue(":emergency_contact_name", trim($d["emergencyName"]));
        $stmt->bindValue(":emergency_contact_relationship", trim($d["emergencyRelationship"]));
        $stmt->bindValue(":emergency_contact_number", trim($d["emergencyContact"]));
        $stmt->bindValue(":reviewed_by", $reviewerId);
        $stmt->bindValue(":converted_student_id", $studentId);
