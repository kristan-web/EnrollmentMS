// admission.js \u2014 the registrar's admission console (registrar/views/admission.html).
...
// wires events. Mirrors accounting/controllers/cashier.js.
(function () {
  "use strict";
  ...
  // The Reports page links here as admission.html#walkin to start a walk-in,
  ...
  statusTabs.addEventListener("click", function (e) {
    ...
    statusTabs.querySelectorAll(".reg-tab").forEach(function (el) {
  ...
  M.requireAuth("../index.html").then(function (user) {
    if (!user) return;
    registrarWho.innerHTML = "<strong>" + esc(user.full_name) + "</strong><span>" + esc(user.role || "Staff") + "</span>";
    loadQueue();
  });
})();
