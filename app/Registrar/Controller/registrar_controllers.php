...
require_once "$projectFilePath/app/Registrar/DAO/RegistrarDAO.php";
require_once "$projectFilePath/app/Students/DAO/StudentDAO.php";
require_once "$projectFilePath/app/Enrollment/DAO/EnrollmentDAO.php";
require_once "$projectFilePath/app/Registrar/Model/admission_model.php";
require_once "$projectFilePath/app/Students/Model/students_model.php";
require_once "$projectFilePath/app/Enrollment/Model/enrollment_model.php";
...
if ($method === "POST") {
    ...
    if ($formType === "login") {
        ...
        echo json_encode([
            ...
            "redirect" => "/EnrollmentMS/app/Registrar/View/admission.php"
        ]);
        exit;
    }
    ...
    if ($formType === "logout") {
        ...
        echo json_encode(["success" => true, "redirect" => "/EnrollmentMS/app/Registrar/View/index.php"]);
        exit;
    }
    ...
    if ($action === "walkin_enroll") {
        ...
        // Record the applicant row that anchors this walk-in in the Registrar
        // Review queue. Without it the student + enrollment exist in the
        // database but never appear in the queue (which lists `applicants`) and
        // can't be finalized (Finalize is keyed on applicant_id). It's stamped
        // 'Approved' and linked to the student, so it lands in the queue ready
        // for the cashier's payment and the registrar's Finalize — the same
        // gate as an online application.
        try {
            $reviewerId = $_SESSION["registrar_user_id"];
            if (!$dao->createWalkInApplicant($_POST, $studentId, $schoolYear["year"], $reviewerId)) {
                fail("The student was enrolled but won't appear in the review queue. Add them from the Enrollment page.");
            }
        } catch (PDOException $e) {
            fail("The student was enrolled but won't appear in the review queue. Add them from the Enrollment page.");
        }
